...
function dbCreator(callback) {
	...
	const marketsArray = ['Fintech', 'Research', 'Security', 'Big Data', 'Telecom', 'Software House'];		

	function createMarkets() {
		for (var y = 0; y < marketsArray.length; y++) {
			const market = new Market({ name: marketsArray[y] });
			market.save()
				.catch((err) => console.warn('Warning: ', err.message));
		}
	}		
